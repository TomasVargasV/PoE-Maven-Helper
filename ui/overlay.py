...
class SequenceDisplay:
    def __init__(self, root, canvas, text_id):
        self.root = root
        self.canvas = canvas
        self.text_id = text_id
        self.visible_color = colors.VISIBLE_COLOR
        self.invisible_color = colors.INVISIBLE_COLOR
    ...
    def set_visible(self, is_currently_visible):
        color = self.visible_color if is_currently_visible else self.invisible_color
        try:
            self.canvas.itemconfig(self.text_id, fill=color)
        except Exception:
            pass
        self.is_visible = is_currently_visible
...
def create_overlay_window(border_thickness=5, border_color="red"):
    ...
    transparent_color = colors.INVISIBLE_COLOR
    ...
    display = SequenceDisplay(root, canvas, text_id)
    return root, display
